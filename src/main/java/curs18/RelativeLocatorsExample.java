...
public class RelativeLocatorsExample extends BaseTest{
	...
	public void relativeLocatorsExample() {
		...
		WebElement author = browser.findElement(RelativeLocator.with
				...
				(price));
		jse.executeScript("arguments[0].setAttribute('style', 'background:yellow; border:4px solid green;')", author);
		...
		jse.executeScript("arguments[0].setAttribute('style', 'background:yellow; border:4px solid green;')", picture);
	}
