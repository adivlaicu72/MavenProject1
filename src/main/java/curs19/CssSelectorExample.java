...
public class CssSelectorExample extends BaseTest {
	...
	public void cssSelectorExample1() {
		...
		//descendant --> adica un spatiu inrtee caracterul parinte si caracterul de tip descendant
		WebElement username = browser.findElement(By.cssSelector("ul[id='menu_user'] input[name='log']"));
		
		jse.executeScript("arguments[0].setAttribute('style', 'background:pink; border:4px solid black;')", username);
		...
		jse.executeScript("arguments[0].setAttribute('style', 'background:yellow; border:4px solid black;')", password);
		
		
		//not()
		WebElement checkbox = browser.findElement(By.cssSelector("ul input:not([type='hidden'],[type='text'],[type='password'],[type='submit'])"));
		jse.executeScript("arguments[0].setAttribute('style', 'background:yellow; border:4px solid black;')", checkbox);
		checkbox.click();
	}
	...
	@Test	
	 public void cssExample2(){
		JavascriptExecutor jse = (JavascriptExecutor)browser;
		 ...
		 jse.executeScript("arguments[0].setAttribute('style', 'background:red; border:4px solid black;')", category1);
		 
		 //last-child
		 WebElement category11 = browser.findElement(By.cssSelector("ul[class='product-categories']>li:last-child"));
		 jse.executeScript("arguments[0].setAttribute('style', 'background:green; border:4px solid black;')", category11);
		 
		 //nth-child()
		 WebElement category5 = browser.findElement(By.cssSelector("ul[class='product-categories']>li:nth-child(5)"));
		 jse.executeScript("arguments[0].setAttribute('style', 'background:yellow; border:4px solid black;')", category5);
		 
		 // selectez din n in n (n = ce vreau eu)
